...
class UpdatePlotThread(threading.Thread):
    ...
    def __init__(self, plot_obj, aux_plot):
    ...
    def run(self):
        while True:
            ...
            if self.calibration_counter.isSet():
                ...
                self.aux_plot.update((roll, pitch, yaw))
...
class CalibrationThread(threading.Thread):
    def __init__(self, plot, update_thread, aux_plot):
    ...
    def run(self):
        ...
        self.eqn_cst["roll"][0] = min(self.aux_plot.data["roll_data"])
        self.eqn_cst["roll"][1] = max(self.aux_plot.data["roll_data"])
        self.eqn_cst["pitch"][0] = min(self.aux_plot.data["pitch_data"])
        self.eqn_cst["pitch"][1] = max(self.aux_plot.data["pitch_data"])
        self.eqn_cst["yaw"][0] = min(self.aux_plot.data["yaw_data"])
        self.eqn_cst["yaw"][1] = max(self.aux_plot.data["yaw_data"])
